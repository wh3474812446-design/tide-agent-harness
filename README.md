...
当前内置模型：**DeepSeek**（OpenAI 兼容协议）。前后端是**同一个 Node 服务**，双击一键启动即可同时跑起来并自动打开浏览器。
...
以后启动：进项目文件夹双击 **`一键启动 Tide`**（前后端一起跑、自动开浏览器；若已在运行则直接打开页面，不会重复启动）。
